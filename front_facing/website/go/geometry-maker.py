...
if array_registered=='true':
    registerf=inputf.replace('submitted_entries.txt','array_register_list')
    info=[q.replace('\n','').split('\t') for q in open(registerf).readlines()]
    Coords=[[float(q[0].split('and')[0]),float(q[0].split('and')[1]),float(q[0].split('and')[2])] for q in info]
    Coords=[str(q[0]+q[2]/2)+':'+str( q[1]+q[2]/2) for q in Coords]
    array_register_dict=dict([(q,'_'+info[i][1]+'_'+info[i][2] )for i, q in enumerate(Coords)])
...
if refIMG=='2':
    ...
    if len(cornerblobs)!=len(untransformed_cornerblobs_saved):
        F=open(outputf,'w')
        F.write('Fiducial point(s) filtered out. They must fall into the distortion correction area as defined by the corner points in the previous image.')
        F.close()
        sys.exit() 
...
print(blobsInfo[0],file=sys.stderr)
...
fname=['PAC_reference.xeo','TLC_reference.xeo'][Platetype=='TLC']
tree = ET.parse('/home/ubuntu/website/go/'+fname)                         
...
for i, (X_coord,Y_coord) in enumerate(coords):
    ...
    if array_registered=='true':
        try:
            new_entry.attrib['PositionName']=str(groupIds[i])+'_'+str(blobs[i][0])+'_'+str(blobs[i][1])+'_'+str((int( str( (blobs[i][0]+1)*123)[2]) +int(blobs[i][1]+1)*51)%(int(blobDimensions[i])*2+10))+'_'+str(blobDimensions[i])+array_register_dict[str(blobs[i][0])+':'+str(blobs[i][1])]
        except:
            continue
    else: 
...
for i, corner_coord in enumerate(corner_coords[:3]):
    new_entry=copy.deepcopy(original_entry)
    new_entry.attrib['PositionIndex']=str(positionindex)
    new_entry.attrib['PositionName']='fiducial_'+str(round(float(corner_coord[0]),2))+'_'+str(round(float(corner_coord[1]),2))+'_null'

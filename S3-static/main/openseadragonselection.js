! function(t) {
    ...
    function e(e) {
        ...
        var i = this.viewer.viewport.deltaPointsFromPixels(e.delta, !0),
            s = this.viewer.viewport.pointFromPixel(e.position, !0),
            r = new t.Point(s.x - i.x, s.y - i.y);
        if (this.rect) {
            var n;
            if (this.restrictToImage && (n = this.rect.clone()), this.rectDone) {
                if (this.allowRotation) {
                    var o = this.rect.getAngleFromCenter(r),
                        h = this.rect.getAngleFromCenter(s);
    ...
    function i() {
        ...
        this.rect.width < 0 && (this.rect.x += this.rect.width, this.rect.width = Math.abs(this.rect.width)), this.rect.height < 0 && (this.rect.y += this.rect.height, this.rect.height = Math.abs(this.rect.height)), this.viewer.setMouseNavEnabled(!0), this.rectDone = !0, this.undraw()

        var i = this.viewer.viewport.viewportToImageRectangle(e);
        i = t.SelectionRect.fromRect(i);
        var gg=String(i).slice(1).split(',');
        var XX=parseFloat(gg[0]);
        var YY=parseFloat(gg[1]);
        var ggg=gg[2].split('@')[0].split('x');
        var ww=parseFloat(ggg[0])
        var hh=parseFloat(ggg[1]) 
    if (fiducial_radio_selected==true){2 }
    if (blob_radio_selected==true){
    
    
    if (document.getElementById('Array_mode')!=null && document.getElementById('Array_mode').checked==false){ 
    if (ww>hh){ 
    gu3(XX+ww/2-hh/2,YY,hh,hh,1,0,'B',0);
    }else{
    gu3(XX,YY+hh/2-ww/2,ww,ww,1,0,'B',0);
    }}
    
    if (document.getElementById('Array_mode')!=null && document.getElementById('Array_mode').checked==true){ 
     
    return null 
    }

    if (document.getElementById('Array_mode')==null){ 
    if (ww>hh){ 
    gu3(XX+ww/2-hh/2,YY,hh,hh,1,0,'B',0);
    }else{
    gu3(XX,YY+hh/2-ww/2,ww,ww,1,0,'B',0);
    ...
    if (typeof roi_set_selected !== 'undefined' && roi_set_selected==true){
    add_ROI(XX ,YY   ,ww,hh,1,0,'R',0);
 
    }
  
 
 
    if (roiremove_radio_selected==true){
     remove_ROI_elements(XX,YY,ww,hh)}
    ...
    function s(e) { 
       ...
       this.rect.width < 0 && (this.rect.x += this.rect.width, this.rect.width = Math.abs(this.rect.width)), this.rect.height < 0 && (this.rect.y += this.rect.height, this.rect.height = Math.abs(this.rect.height)), this.viewer.setMouseNavEnabled(!0), this.rectDone = !0, this.undraw()       
        ...
        if (isrect==0 && ismouseover==0){
        var webPoint = e.position;
        var viewportPoint = viewer.viewport.pointFromPixel(webPoint);
        var imagePoint = viewer.viewport.viewportToImageCoordinates(viewportPoint);
        var clickpos=imagePoint.toString().slice(1, -1).split(','); 
        var XXX=parseFloat(clickpos[0]);
        var YYY=parseFloat(clickpos[1]);
        var ww=boxsize*0.1
        var hh=boxsize*0.1
        if (fiducial_radio_selected==true){add_fiducial(XXX-0.5,YYY-0.5,1,1,1,0,'F',0)}
        if (document.getElementById('Array_mode')!=null){ 
        if (blob_radio_selected==true && document.getElementById('Array_mode').checked==false){gu3(XXX-0.5,YYY-0.5,1,1,1,0,'B',0)} 
        if (blob_radio_selected==true && document.getElementById('Array_mode').checked==true){add_corner_blob_array(XXX-0.5,YYY-0.5,1,1,1,0,'B',0)} }

        if (document.getElementById('Array_mode')==null && fiducial_radio_selected==false ){gu3(XXX-0.5,YYY-0.5,1,1,1,0,'B',0)}

                };
        this.viewer.canvas.focus();
        //this.viewer.viewport.zoomTo(this.viewer.viewport.getZoom(true)+0.001);
        isrect=0;
    ...
    function o(e, i) {
        var s, r = i.delta,
            n = this.rect.getDegreeRotation(),
            o = this.restrictToImage ? this.rect.clone() : null;
        switch (0 !== n && (r = r.rotate(-1 * n, new t.Point(0, 0)), s = this.rect.getCenter()), r = this.viewer.viewport.deltaPointsFromPixels(r, !0), e) {
            case 0:
    ...
    function l(t) {
        ...
        13 === e ? this.confirm() : String.fromCharCode(e) === this.keyboardShortcut && this.toggleState()
    }
    ...
    function a(e, i, s) {
        ...
        var n = i.minus(e),
            o = e.distanceTo(i),
            h = -1 * Math.atan2(n.x, n.y) + Math.PI / 2,
            l = new t.Point(n.x / 2 + e.x, n.y / 2 + e.y),
            a = new t.SelectionRect(l.x - o / 2, l.y - s / 2, o, s, h),
            c = new t.Point(0, s);
    ...
    t.Viewer.prototype.selection = function(e) {
        return this.selectionInstance && !e || (e = e || {}, e.viewer = this, this.selectionInstance = new t.Selection(e)), this.selectionInstance
    }, t.Selection = function(a) {
        t.extend(!0, this, {
            viewer: null,
            isSelecting: !1,
            buttonActiveImg: !1,
        ...
        var u = this.prefixUrl || this.viewer.prefixUrl || "",
            w = this.viewer.buttons && this.viewer.buttons.buttons,
            y = w ? this.viewer.buttons.buttons[0] : null,
            m = y ? y.onFocus : null,
            v = y ? y.onBlur : null;
        if (this.showSelectionControl && (this.toggleButton = new t.Button({
            ...
            p.classList.add("confirm-button"), this.element.appendChild(p), this.cancelButton = new t.Button({
                ...
                clickTimeThreshold: this.viewer.clickTimeThreshold,
                clickDistThreshold: this.viewer.clickDistThreshold,
                tooltip: t.getString("Tooltips.SelectionCancel") || "Cancel selection",
        ...
        setState: function(t) {
            return this.isSelecting = t, this.outerTracker.setTracking(t), t ? this.draw() : this.undraw(), this.buttonActiveImg && (this.buttonActiveImg.style.visibility = t ? "visible" : "hidden"), this.viewer.raiseEvent("selection_toggle", {
                enabled: t
            }), this
        },
        setAllowRotation: function(t) {
            this.allowRotation = t
        },
        enable: function() {
        ...
        draw: function() {
            return this.rect && (this.overlay.update(this.rect.normalize()), this.overlay.drawHTML(this.viewer.drawer.container, this.viewer.viewport)), this
        },
        undraw: function() {
...
function(t) {
    "use strict";
    t.SelectionOverlay = function(e, i) {
        t.Overlay.apply(this, arguments), t.isPlainObject(e) ? this.rotation = e.location.rotation || 0 : this.rotation = i.rotation || 0
    }, t.SelectionOverlay.prototype = t.extend(Object.create(t.Overlay.prototype), {
        drawHTML: function() {
            t.Overlay.prototype.drawHTML.apply(this, arguments), this.style.transform = this.style.transform.replace(/ ?rotate\(.+rad\)/, "") + " rotate(" + this.rotation + "rad)"
        },
        update: function(e) {
            t.Overlay.prototype.update.apply(this, arguments), this.rotation = e.rotation || 0
        }
    })
}(OpenSeadragon),
function(t) {
    "use strict";
    t.SelectionRect = function(e, i, s, r, n) {
        t.Rect.apply(this, [e, i, s, r]), this.rotation = n || 0
    }, t.SelectionRect.fromRect = function(e) {
        return new t.SelectionRect(e.x, e.y, e.width, e.height)
    }, t.SelectionRect.prototype = t.extend(Object.create(t.Rect.prototype), {
        clone: function() {
            return new t.SelectionRect(this.x, this.y, this.width, this.height, this.rotation)
        },
        equals: function(e) {
            return t.Rect.prototype.equals.apply(this, [e]) && this.rotation === e.rotation
        },
        toString: function() {
        ...
        round: function() {
            return new t.SelectionRect(Math.round(this.x), Math.round(this.y), Math.round(this.width), Math.round(this.height), this.rotation)
        },
        normalize: function() {
            var t = this.clone();
        ...
        fitsIn: function(t) {
            for (var e = this.normalize(), i = [e.getTopLeft(), e.getTopRight(), e.getBottomRight(), e.getBottomLeft()], s = e.getCenter(), r = e.getDegreeRotation(), n = t.getBottomRight(), o = 0; o < 4; o++)
                if (i[o] = i[o].rotate(r, s), i[o].x < t.x || i[o].x > n.x || i[o].y < t.y || i[o].y > n.y) return !1;
            return !0
        },

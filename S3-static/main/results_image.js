    ...
    function clearup2(n){
     ...
     if (n==1){ document.getElementById("Blobgroups").value=''
                document.getElementById("Bloblist").value=''}
     if (n==2){ document.getElementById("Blobs").value=''
                document.getElementById("Bloblist").value=''}
     if (n==3){ document.getElementById("Blobs").value=''
                document.getElementById("Blobgroups").value=''}}else{
    ...
    var movetarget=[]
	    for (i = 0; i <blobs.length; i++) {   
		    if (blobs[i][blobgroupswitch]==document.getElementById('Blobs').value){
                    ...
                    var entry=[blobs[i][blobgroupswitch+1],blobs[i][blobgroupswitch+2],'16','16','0','0',data,blobs[i][blobgroupswitch]]
    ...
    entries = entries.filter( onlyUnique );
            ...
            for (i = 0; i <blobs.length; i++) {
                    ...
                    var data= blobs[i].slice(blobgroupswitch+3,blobs[i].length-1).join('-')
                    var entry=[blobs[i][blobgroupswitch+1],blobs[i][blobgroupswitch+2],'16','16','0','0',data,blobs[i][1]]
		    out.push(entry.join('and') )                          
		                         }
            }
            }
...
function loadFile(filePath) {    
  ...
  xmlhttp.open("GET", 'https://tmacroms.s3.us-east-2.amazonaws.com/out_{{ ID }}/'+filePath, false);
   ...
   if (isNumeric(entries[count])==true){
    ...
    if(Blobgroups.length>1){
     ...
     var fs=[]
      ...
      var range_title='with grayed out m/z range: '
      ...
      for (ii = 0; ii <indexes.length; ii++) {
        ...
        x_Ranges.push([parseFloat(mzranges[indexes[ii]].split('-')[0])-Range,parseFloat(mzranges[indexes[ii]].split('-')[1])+Range]) 
        if (ii==0){
         showswitch=true } 
        var subplot = {
 	x: X,
  	...
  	type: 'scatter',
        ...
        name: '['+filtered_blobns[i]+'] '+filtered_fnames[i],
        line: {
        color: colors[i%10],
        width: 3
        ...
        if (i==0){
         ...
         var Shape=        {
            ...
            range_title+=mzranges[indexes[ii]]+' '
...
jSON="var layout = {legend: {itemclick: false,itemdoubleclick: false}, title: {text:'MALDI-ToF-MS intensity vs m/z plot "+range_title+"'}"+",grid: {rows: fs.length,columns: indexes.length, pattern: 'independent', }, shapes: shapes_layout,"
...
var config
config= {displaylogo: false , modeBarButtonsToRemove: ['sendDataToCloud','lasso2d','hoverClosestCartesian', 'hoverCompareCartesian','select2d','autoScale2d','toggleSpikelines'],
...
Plotly.newPlot('section2', subplots, layout,  config);
  ...
  var textbox = document.createElement("div")
      ...
      textbox.style= "box-sizing: border-box;  "
    ...
    var viewer = OpenSeadragon({
      ...
      tileSources: 'https://tmacroms.s3.us-east-2.amazonaws.com/out_{{ ID }}/submitted_dzi.dzi',
      maxZoomLevel: 10, 
      minZoomLevel: .5,
      defaultZoomLevel: 1,

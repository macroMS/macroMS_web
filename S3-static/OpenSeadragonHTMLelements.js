(function() {
  ...
  $.hElements.prototype = {
	...
	var dat=e.id.split('and')
	 var mesg='Sum intensity data <br> '
	 var mesg2='Sum intensity data \n'

	for (i = 0; i <mzranges.length; i++) {  
	mesg+='m/z '+mzranges[i]+': '+dat[dat.length-2].split('-')[i] +"<br><br>"
	mesg2+='m/z '+mzranges[i]+': '+dat[dat.length-2].split('-')[i] +"\n"
	
	};
	//status_update(mesg)
	wrapperDiv.title=mesg2
    ...
    removeElementById: function(id) {
      const e = this.getElementById(id).element;
	            remove_element(id);
    ...
    goToElementLocation: function(id) {
      ...
      const vpPos = viewer.viewport.imageToViewportCoordinates(e.rect.x, e.rect.y)
        this.viewer.viewport.fitBoundsWithConstraints(new OpenSeadragon.Rect(
          vpPos.x - vpRect.width*20 / 2,
          vpPos.y - vpRect.height*20 / 2,
          vpRect.width*20,
          vpRect.height*20
        ))
      }

...
#        self.slide.size, self.slide.pos = size, pos 
    ...
    def saveCurrentBlobFinding(self, filename,TSPopt,ID=None,csrf=None):
        '''
...
#            return "No slide loaded"
        ...
        if TSPopt==True:
            ...
            soln = TSPutil.TSPRoute(blob.blob.getXYList(blobs),ID,csrf)
            blobs = [blobs[i] for i in soln]
            self.blobCollection[self.currentBlobs].blobs=blobs
    ...
    def savePackedBlobs(self, filename):
        '''
        Save the packed blob list with group number, creating unconventional blobs file with group number attached to each blob
        filename: file to save to
        '''
        if self.blobCollection[self.currentBlobs].length() == 0:
            return "List {} contains no blobs!".format(self.currentBlobs +1) #plus one for GUI display
        self.blobCollection[self.currentBlobs].saveBlobs2(filename)
        return "Saved blob information of list {}".format(self.currentBlobs+1)
...
#        return self.coordinateMapper.instrumentExtension
    ...
    def runGlobalBlobFind(self,minSize = 50, maxSize = None,
        ...
        self.blobCollection[self.currentBlobs].setParameters(minSize, maxSize, minCircularity, maxCircularity, colorChannel, threshold, imageIndex)
        self.blobCollection[self.currentBlobs].blobSlide() 
        return self.blobCollection[self.currentBlobs].showBlobs() 
...
#        return self.blobCollection[self.currentBlobs].blobSlide() + " in list {}".format(self.currentBlobs+1)
    ...
    def distanceFilter(self, distance,ID):
        '''
        ...
        if distance==None:
            return self.blobCollection[self.currentBlobs].minimumDistances(ID)
...
#        return "{} blobs removed, {} remain in list {}".format(startLen - endLen, endLen, self.currentBlobs+1)
    ...
    def circularPackBlobs(self, separation, maxShots, offset):
        ...
        return self.blobCollection[self.currentBlobs].showBlobs()  ### line added without replacing any original code!!!!!
#    def analyzeAll(self):
#        '''
